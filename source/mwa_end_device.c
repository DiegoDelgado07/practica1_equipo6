...
void SendCounterPacket();//manda el paquete de contador

void App_init( void );
void AppThread (osaTaskParam_t argument);
void timer_count (osaTaskParam_t argument);
...
void incrementarDireccion();

static uint8_t associate_flag = 0;
/************************************************************************************
*************************************************************************************
...
OSA_TASK_DEFINE( timer_count, mAppTaskPrio_c, 1, mAppStackSize_c, FALSE );
...
uint64_t mExtendedAddress  = mMacExtendedAddress_c;

void incrementarDireccion() {//para estar cambiando de extended address
	mExtendedAddress++;
...
void App_init( void )
{
    ...
    incrementarDireccion();//para estar cambiando de extended address
    //Mac_SetExtendedAddress( (uint8_t*)&mExtendedAddress, macInstance );
                ...
                {   
                    ...
                    {          
                        ...
                        { 
                            ...
                            Serial_Print(interfaceId, "Extended address is 0x", gAllowToBlock_d);//extended
                            Serial_PrintHex(interfaceId, (uint8_t*)&mExtendedAddress, 8, gPrtHexNoFormat_c);
                            Serial_Print(interfaceId, "\n\r", gAllowToBlock_d);
    ...
    while (NULL != pDescBlock)
    {
      ...
      {            
        ...
        if( ( pPanDesc->superframeSpec.associationPermit ) && 
            ( pPanDesc->superframeSpec.beaconOrder == 0x0F)  && (pPanDesc->logicalChannel == 26) ) //revisa que sea el canal 26
        {        
...
static uint8_t App_HandleAssociateConfirm(nwkMessage_t *pMsg)
{
  ...
  if ( pMsg->msgData.associateCnf.status == gSuccess_c) 
  {
	  ...
	  MyTaskTimer_Start();//en cuanto se asocia comienza timer
	  associate_flag = 1;
	  return gSuccess_c;
  } 
...
void intToStr(uint8_t number, char* buffer) {
    uint8_t i = 0;
    uint8_t j;
    char temp[3];
    do {
...
void SendCounterPacket()
{
    ...
    if( (mcPendingPackets < mDefaultValueOfMaxPendingDataPackets_c) && (mpPacket == NULL) )
    {
    	/* If the maximum number of pending data buffers is below maximum limit
          and we do not have a data buffer already then allocate one. */
        mpPacket = MSG_Alloc(sizeof(nwkToMcpsMessage_t) + sizeof("Counter:x"));

        if(mpPacket != NULL)
        {

			mpPacket->msgType = gMcpsDataReq_c; // data request
			mpPacket->msgData.dataReq.pMsdu = (uint8_t*)(&mpPacket->msgData.dataReq.pMsdu) +
											  sizeof(mpPacket->msgData.dataReq.pMsdu);

			char message[16];
			snprintf(message, sizeof(message), "Counter:%d", GetCounter());
			strncpy((char*)mpPacket->msgData.dataReq.pMsdu, message, sizeof(message));

			Serial_Print(interfaceId, "Sending: ", gAllowToBlock_d);
			Serial_Print(interfaceId, (char*)mpPacket->msgData.dataReq.pMsdu, gAllowToBlock_d);
			Serial_Print(interfaceId, "\r\n", gAllowToBlock_d);
			/* Create the header using coordinator information gained during
               the scan procedure. Also use the short address we were assigned
               by the coordinator during association. */
            mpPacket->msgData.dataReq.msduLength = strlen((char*)mpPacket->msgData.dataReq.pMsdu);
            FLib_MemCpy(&mpPacket->msgData.dataReq.dstAddr, &mCoordInfo.coordAddress, 8);
            FLib_MemCpy(&mpPacket->msgData.dataReq.srcAddr, &maMyAddress, 8);
            FLib_MemCpy(&mpPacket->msgData.dataReq.dstPanId, &mCoordInfo.coordPanId, 2);
            FLib_MemCpy(&mpPacket->msgData.dataReq.srcPanId, &mCoordInfo.coordPanId, 2);
            ...
            /* Don't use security */
            mpPacket->msgData.dataReq.securityLevel = gMacSecurityNone_c;
            NWK_MCPS_SapHandler(mpPacket, macInstance); // perform the data request
