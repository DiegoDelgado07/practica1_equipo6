...
static const uint16_t mPanId = 0x6666;
...
typedef struct {
    ...
    bool rxOnWhenIdle;
    bool deviceType; // true -> FFD, false- RFD
} Infonode_t;

Infonode_t nodes[NODES];//info de 5 nodos
uint8_t node_contador = 0;
...
#else      
  /* Select default channel */
  mLogicalChannel = 26;//directamente usar este canal
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  uint64_t extended_address = pMsgIn->msgData.associateInd.deviceAddress;
  Serial_Print(interfaceId, "Extended Address recibido: 0x", gAllowToBlock_d);//para verificar
  Serial_PrintHex(interfaceId, (uint8_t*)&extended_address, 8, gPrtHexNoFormat_c);
  Serial_Print(interfaceId, "\n\r", gAllowToBlock_d);
  bool old_node = false;
  uint16_t assigned_short_address = 0xFFFF;

  Serial_Print(interfaceId,"Sending the MLME-Associate Response message to the MAC...", gAllowToBlock_d);

  for (uint8_t i = 0; i < node_contador; i++) { //busca si ya estaba el nodo anteriomente
        if (nodes[i].extendedAddress == extended_address) {
        	assigned_short_address = nodes[i].shortAddress;
            old_node = true;
            break;
        }
    }

  if (!old_node){ //en caso que sea nodo nuevo, asignar short address nuevo
	  if(node_contador < NODES){
		  assigned_short_address = node_contador + 1;
		  nodes[node_contador].shortAddress = assigned_short_address;
		  nodes[node_contador].extendedAddress = extended_address ;
		  nodes[node_contador].rxOnWhenIdle = (pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoRxWhenIdle_c) != 0;
		  nodes[node_contador].deviceType = (pMsgIn->msgData.associateInd.capabilityInfo & gCapInfoDeviceFfd_c) != 0;
		  node_contador++;

	  }else {
		  Serial_Print(interfaceId, "Limite de nodos\n", gAllowToBlock_d);
	  }
  ...
  if(pMsg != NULL)
  {
    ...
    pAssocRes->assocShortAddress = assigned_short_address;
 ...
 //   pAssocRes->securityLevel = gMacSecurityNone_c;
    ...
    {//muestra esto en teraterm
    	Serial_Print(interfaceId, "Nodo asociado con: 0x", gAllowToBlock_d);
    	Serial_PrintHex(interfaceId, (uint8_t *)&assigned_short_address, 2, gPrtHexNoFormat_c);
    	Serial_Print(interfaceId, "\n\r", gAllowToBlock_d);


    	Serial_Print(interfaceId, "RxOnWhenIdle: ", gAllowToBlock_d);//rxonwhenidel
    	if (nodes[node_contador - 1].rxOnWhenIdle) {
    	...
    	Serial_Print(interfaceId, "DeviceType: ", gAllowToBlock_d);//revisar tipo de device
    	if (nodes[node_contador - 1].deviceType) {
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
      ...
      uint8_t *payload = pMsgIn->msgData.dataInd.pMsdu; // Point to the payload
      uint8_t payloadLength = pMsgIn->msgData.dataInd.msduLength;

      // Source address of the message
      uint16_t srcAddress = pMsgIn->msgData.dataInd.srcAddr;

      // LQI of the message
      uint8_t lqi = pMsgIn->msgData.dataInd.mpduLinkQuality;

      // Print source address, LQI, and payload size
      Serial_Print(interfaceId, "Source Address: 0x", gAllowToBlock_d);
      Serial_PrintHex(interfaceId, (uint8_t*)&srcAddress, sizeof(srcAddress), gPrtHexNoFormat_c);

      Serial_Print(interfaceId, " LQI: ", gAllowToBlock_d);
      Serial_PrintDec(interfaceId, lqi);

      Serial_Print(interfaceId, " Payload Size: ", gAllowToBlock_d);
      Serial_PrintDec(interfaceId, payloadLength);
      Serial_Print(interfaceId, "\r\n", gAllowToBlock_d);

      // Check if the payload starts with "Counter:"
      if (payloadLength > 8 && strncmp((char*)payload, "Counter:", 8) == 0)
      {
          uint8_t receivedCounter = payload[8] - '0'; // Get the counter value

          LEDS_color(receivedCounter); // Update LED state
      }
